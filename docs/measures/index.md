...
The `rawdata/phenotype/` folder includes tabulated *Basic Demographics* (`sed_basic_demographics`) and *Visit Data* (`par_visit_data`) associated with the HBCD study. Basic Demographics is a derived category gathering data from multiple sources, including [*HBCD Demographics V01*](socenvdet.md#hbcd-demographics) (`sed_bm_demo`) as well as some calculated fields for parent and child race & ethnicity.
